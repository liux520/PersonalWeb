...
const typeColors = {
  publication: { bg: 'bg-blue-100 dark:bg-blue-900', text: 'text-blue-700 dark:text-blue-200', border: 'border-blue-200 dark:border-blue-800' },
  talk: { bg: 'bg-purple-100 dark:bg-purple-900', text: 'text-purple-700 dark:text-purple-200', border: 'border-purple-200 dark:border-purple-800' },
  award: { bg: 'bg-amber-100 dark:bg-amber-900', text: 'text-amber-700 dark:text-amber-200', border: 'border-amber-200 dark:border-amber-800' },
  grant: { bg: 'bg-emerald-100 dark:bg-emerald-900', text: 'text-emerald-700 dark:text-emerald-200', border: 'border-emerald-200 dark:border-emerald-800' },
}
    ...
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-slate-50 to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      ...
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <h1 className="text-xl font-bold text-gradient">All News</h1>
        </div>
      </motion.header>
        ...
        >
          {(['all', 'publication', 'talk', 'award', 'grant'] as const).map((filterType) => (
            ...
            >
              ...
              {filterType === 'publication' && `Publications (${counts.publication})`}
            ...
            .map(([year, yearNews], yearIndex) => (
              ...
              >
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-teal-600 to-amber-500 flex items-center justify-center">
                  ...
                  <span className="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-full text-sm font-medium">
                    {yearNews.length} {yearNews.length === 1 ? 'item' : 'items'}
                  </span>
                </div>
                  ...
                  {yearNews.map((item, index) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="bg-white dark:bg-slate-800 rounded-xl p-5 shadow-md hover:shadow-lg transition-all duration-300 relative overflow-hidden"
                      >
                        <div className={`absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b ${getTypeStyles(item.type)}`} />
                        <div className="flex items-start gap-4">
                          <div className={`flex-shrink-0 px-3 py-1.5 rounded-lg bg-gradient-to-br ${getTypeStyles(item.type)} text-white text-sm font-semibold`}>
                            {item.date}
                          </div>
